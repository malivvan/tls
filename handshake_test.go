...
func runMain(m *testing.M) int {
	...
	// Go 1.26 ignores user-provided readers for ECDHE key generation by
	// default (crypto/internal/rand.CustomReader). Tests rely on the
	// deterministic zeroSource reader, so opt back into honoring it, as
	// stdlib crypto/tls does.
...
func testUtlsHandshake(t *testing.T, clientConfig, serverConfig *Config, spec *ClientHelloSpec) (serverState, clientState ConnectionState, err error) {
	...
	go func() {
		...
		if spec != nil {
			ucli := UClient(c, clientConfig, HelloCustom, false, false, false)
